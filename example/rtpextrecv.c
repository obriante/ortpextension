...
main (int argc, char *argv[])
{
  ...
  rtp_session_signal_connect (session, "ssrc_changed", (RtpCallback) ssrc_cb,
			      0);
  rtp_session_signal_connect (session, "ssrc_changed",
  ...
  while (cond) {
	...
	(RtpExtension_t *) init_rtp_extension ();
      set_extension (rtp_extension, 0x67, NULL, 0);

      err = rtp_session_recv_with_ts_with_extension (session, buffer, 160,
						     ts, &have_more,
						     rtp_extension);
	...
	if (sound_fd > 0) {
	  ...
	  if (ret == -1) {
	    fprintf (stderr, "write to sound card failed (%s)",
		     strerror (errno));
	  }
	}
	contatore++;
