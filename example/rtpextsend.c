...
int main(int argc, char *argv[]) {
	...
	for (i = 4; i < argc; i++) {
		if (strcmp(argv[i], "--with-clockslide") == 0) {
			...
			ortp_message("Using clockslide of %i milisecond every 50 packets.",
					clockslide);
	...
	/*uint32_t value[2];
	 value[0]=5;
	 value[1]=6;
	 set_extension(rtp_extension,0x67,value,2);*/

	uint32_t value[1];
	value[0] = 5;
	set_extension(rtp_extension, 0x67, value, 1);
	...
	while (((i = fread(buffer, 1, 160, infile)) > 0) && (runcond)) {
		...
		rtp_session_send_with_ts_with_extension(session, buffer, i, user_ts,
				rtp_extension);
