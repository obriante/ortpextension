 ...
 **/
int rtp_session_send_with_ts_with_extension(RtpSession * session,
		const uint8_t * buffer, int len, uint32_t userts,
...
#else
	m = rtp_session_create_packet_with_extension(session, RTP_FIXED_HEADER_SIZE,
			(uint8_t*) buffer, len, extension);
#endif
...
void rtp_header_init_from_session_with_extension(rtp_header_t *rtp,
		RtpSession *session) {
	rtp->version = 2;
	rtp->padbit = 0;
	rtp->extbit = 1;
...
mblk_t * rtp_session_create_packet_with_extension(RtpSession *session,
		int header_size, const uint8_t *payload, int payload_size,
		RtpExtension_t *extension) {
 ...
 * @return a rtp packet in a mblk_t (message block) structure.
 */
mblk_t * rtp_session_create_packet_with_data_with_extension(RtpSession *session,
		uint8_t *payload, int payload_size, void (*freefn)(void*),
		RtpExtension_t *extension) {
	mblk_t *mp, *mpayload;
 ...
 */
int rtp_get_payload_with_extension(mblk_t *packet, unsigned char **start,
 ...
 *
 **/
int rtp_session_recv_with_ts_with_extension(RtpSession * session,
 ...
 */
void set_jitter_buffer(RtpSession *session, int nomsize, bool adaptative,
		int max_pkts) {
