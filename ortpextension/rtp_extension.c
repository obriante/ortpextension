 ...
 * @return Pointer to a initialized RTP Extension
 * */
void *init_rtp_extension_with_length(uint16_t length){
 ...
 * */
void uninit_rtp_extension(RtpExtension_t *rtp_extension){
 ...
 */
void set_extension(RtpExtension_t *rtp_extension, uint16_t type, uint32_t* words, uint16_t length){
	...
	if(length>0){
		...
		if(words!=NULL){
			...
			rtp_extension->length=length;
			rtp_extension->extension=(uint32_t*)malloc(length*sizeof(uint32_t));
 ...
 **/
void* rtp_extension_to_uint32_t(RtpExtension_t* extension){

	uint32_t *extension_buf=(uint32_t *)malloc(sizeof(uint32_t)*(extension->length+1));
	...
	if(extension->extension!=NULL){
		...
		for(i=extension->length-1; i>=0; i--){
			...
			memcpy(&extension_buf[i+1],&extension->extension[i],sizeof(uint32_t));
 ...
 */
void print_rtp_extension(RtpExtension_t* rtp_extension){
