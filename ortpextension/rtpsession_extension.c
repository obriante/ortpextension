 ...
 **/
int rtp_session_send_with_ts_with_extension (RtpSession * session, const uint8_t * buffer, int len, uint32_t userts, RtpExtension_t *extension)
{
	mblk_t *m;
	int err;

#ifdef USE_SENDMSG
	m=rtp_session_create_packet_with_data_with_extension(session,(uint8_t*)buffer,len,NULL, extension);
#else
	m = rtp_session_create_packet_with_extension(session,RTP_FIXED_HEADER_SIZE,(uint8_t*)buffer,len, extension);
#endif
...
void rtp_header_init_from_session_with_extension(rtp_header_t *rtp, RtpSession *session){
	rtp->version = 2;
	rtp->padbit = 0;
	rtp->extbit = 1;
	rtp->markbit= 0;
 ...
 **/
mblk_t * rtp_session_create_packet_with_extension(RtpSession *session,int header_size, const uint8_t *payload, int payload_size, RtpExtension_t *extension){

	mblk_t *mp;
	uint32_t *rtp_extension=(uint32_t *)rtp_extension_to_uint32_t(extension);
	uint8_t *rtp_ext=(uint8_t *)bytes_inverter_to_packet(rtp_extension,extension->length+1);
 ...
 * @return a rtp packet in a mblk_t (message block) structure.
 */
mblk_t * rtp_session_create_packet_with_data_with_extension(RtpSession *session, uint8_t *payload, int payload_size, void (*freefn)(void*), RtpExtension_t *extension)
{
	mblk_t *mp,*mpayload;
 ...
 */
int rtp_get_payload_with_extension(mblk_t *packet, unsigned char **start, RtpExtension_t *rtp_extension){
	...
	if (rtp_get_extbit(packet)){
		...
		if (extsize>=0){
			...
			words=(uint8_t*)bytes_inverter_from_packet((uint32_t*)words, rtp_extension->length);
 ...
 *
 **/
int rtp_session_recv_with_ts_with_extension(RtpSession * session, uint8_t * buffer,
		int len, uint32_t ts, int * have_more, RtpExtension_t *rtp_extension){
	mblk_t *mp=NULL;
	int plen,blen=0;
	*have_more=0;
	...
	while(1){
		if (session->pending){
			...
			if (mp!=NULL) rtp_get_payload_with_extension(mp,&mp->b_rptr,rtp_extension);
		}
		if (mp){
 ...
 */
void set_jitter_buffer(RtpSession *session, int nomsize, bool adaptative, int max_pkts ){
